...
const createUser = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "User succesvol aangemaakt", user: newUser });
  } catch (error) {
    console.error("Error creating user:", error);
    if (error.code === "P2002" || error.message.includes("validation failed")) {
      return res.status(400).json({
        message: "Validatiefout bij het aanmaken van de gebruiker",
...
const updateUser = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "User succesvol bijgewerkt", user: updatedUser });
  } catch (error) {
    console.error("Error updating user:", error);
    if (error.code === "P2002" || error.message.includes("validation failed")) {
      return res.status(400).json({
        message: "Validatiefout bij het bijwerken van de gebruiker",
        details: error.meta || error.message,
      });
    }
    res.status(500).json({
...
const deleteUser = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "User succesvol verwijderd", user: deletedUser });
  } catch (error) {
